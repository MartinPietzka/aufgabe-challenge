...
public class RepositoryJdbcImpl implements Repository {
    ...
    private Map<Long, Bankkonto> kontos = new HashMap<>();
    ...
    public Optional<Bankkonto> findBankkontoByKontonummer(long kontonummer) {
        if (kontos.get(kontonummer) != null) return Optional.of(kontos.get(kontonummer));
        ...
        try (Connection con = dataSource.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                kontos.put(kontonummer, bankkonto);
    ...
    public <T> void save(T obj) {
        ...
        if (con == null) throw new IllegalStateException("Keine Connection vorhanden.");
        if (obj == null) throw new IllegalArgumentException("obj darf nicht null sein.");

        if (obj instanceof Bankkonto) {
            ...
            try (PreparedStatement ps = con.prepareStatement(updateKontostand)) {
                ...
                throw new RuntimeException("Update Bankkonto fehlgeschlagen", e);
            }
        }

        if (obj instanceof Geldautomat) {
            Geldautomat geldautomat = (Geldautomat) obj;
            String updateBargeld = "UPDATE geldautomat SET bargeld = ? WHERE automatennummer = ?";
            try (PreparedStatement ps = con.prepareStatement(updateBargeld)) {
                ps.setBigDecimal(1, geldautomat.ermittleVerfuegbaresBargeld());
                ps.setLong(2, geldautomat.getNummer());
                ps.execute();
            } catch (SQLException e) {
                throw new RuntimeException("Update Geldautomat fehlgeschlagen", e);
            }
        }
    }
